...
def list_tables_from_folder():
    """Lists tables from the first Word document in the selected folder."""
    ...
    # Find first .docx file in the folder
    doc_files = [f for f in os.listdir(folder_path) if f.endswith(".docx")]
    
    if not doc_files:
        messagebox.showerror("Error", "No Word documents found in the selected folder.")
        return
    
    first_doc_path = os.path.join(folder_path, doc_files[0])
    list_tables(first_doc_path) # Use the first document for table preview

def list_tables(doc_path):
    """Extracts tables from a document and shows their index with a preview."""
    try:
...
def select_folder():
    ...
    list_tables_from_folder() # Show table preview from the first document
...
# GUI Window
root = tk.Tk()
root.title("ESCP Chemical Table Replace")
